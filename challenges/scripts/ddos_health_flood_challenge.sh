...
# MCP_Module per CONST-035 + CONST-050(B). Submodule cascade per
# CONST-051(A). Targets $MCP_MODULE_HEALTH_URL.
...
echo "=== MCP_Module DDoS Health-Flood Challenge ==="
echo "  url=$HEALTH_URL total=$TOTAL_REQS conc=$CONCURRENCY pass≥${MIN_PASS_PCT}%"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/5] SKIP: MCP_MODULE_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== MCP_Module DDoS Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== MCP_Module DDoS Challenge: PASSED ==="
