# CONSTITUTION -- digital.vasic.mcp (MCP_Module)

This Constitution governs the `digital.vasic.mcp` Go module (generic,
reusable MCP protocol implementation). It inherits, in full, the
HelixAgent root Constitution (`HelixAgent/CONSTITUTION.md`) and the
universal mandatory rules below. Module-specific addenda may extend but
...
- **Language**: Go 1.24+
- **Primary consumer**: HelixAgent (`dev.helix.agent`) and HelixLLM
- **Source of truth**: this repository, mirrored to GitHub + GitLab
  (`vasic-digital/MCP_Module`)

## Module-Specific Constitutional Rules

1. **Interface stability.** `client.Client`, `server.Server`,
   `server.ToolHandler`, `registry.Adapter` are public contracts.
   Signature changes require a major-version bump and synchronized
   updates of every implementation and call site.
2. **Protocol package is foundational.** `pkg/protocol` is imported by
   every other package. Changes there require running
   `go test ./... -count=1 -race` across the full module before commit.
3. **Transport symmetry.** The stdio and HTTP/SSE transports MUST mirror
   each other. Adding a feature to one transport requires a matching
   change in the other.
4. **Concurrency contracts.** Existing `sync.Mutex` / `sync.RWMutex` /
   `atomic.Int64` guarantees on `StdioClient`, `HTTPClient`,
   `HTTPServer`, and `Registry` MUST be preserved. New shared mutable
   state must adopt the project's safe-container primitives where
   available (see Universal rule #13).
5. **Wire-compatibility freeze.** JSON struct tags on `pkg/protocol`
   types and the `JSONRPCVersion = "2.0"` constant are immutable.
